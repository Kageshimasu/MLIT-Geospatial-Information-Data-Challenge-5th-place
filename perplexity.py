...
non_binary_columns = ['target_ym', 'money_room', 'lat', 'lon', 'unit_area', 'addr1_1', 'addr1_2', 'convenience_distance', 'walk_distance1']
...
# バイナリカラムのみを使用してドキュメント行列を作成
docs = tokyo_data_sampled[binary_columns].values

# データを訓練・テストに分割
from sklearn.model_selection import train_test_split
train_docs, test_docs = train_test_split(docs, test_size=test_ratio, random_state=random_seed)

# Torchテンソルに変換
train_docs = torch.tensor(train_docs, dtype=torch.float32)
test_docs = torch.tensor(test_docs, dtype=torch.float32)

# 単語辞書の作成（カラム名を単語として使用）
...
print('Test Corpus size: {}'.format(test_docs.shape))
...
for num_topics in topic_nums:
    ...
    test_loss = 0.0
    with torch.no_grad():
        num_test_batches = int(math.ceil(test_docs.shape[0] / batch_size))
        for i in range(num_test_batches):
            batch_docs = test_docs[i * batch_size:(i + 1) * batch_size, :]
            loss = svi.evaluate_loss(batch_docs)
            test_loss += loss  # 損失の合計を保持

    # パープレキシティの計算
    total_words = test_docs.sum().item()
    perplexity = math.exp(test_loss / total_words)  # math.exp()を使用
    ...
    # svi_samples = predictive(docs)

    # logthetaを取得し、thetaを計算
    # logtheta = svi_samples["logtheta"].squeeze(0)  # num_samples=1なので次元を削減
    # theta = F.softmax(logtheta, dim=-1).detach().cpu().numpy()

    # トピックの割合をDataFrameに変換
    ...
    # tokyo_data_with_topics.to_csv(f"./tokyo_data_with_topics_4_topic{num_topics}.csv", index=False)
    ...
    #     ax.axis('off')
    # plt.tight_layout()
    # plt.savefig(f"./word_cloud_4_topic{num_topics}.png")
